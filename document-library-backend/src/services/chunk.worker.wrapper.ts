...
export const runChunkWorker = async (originalDocument: OriginalDocument, owner: Owner): Promise<Chunk[]> => {

  try {
    const extractedChunks = await handleDocumentChunking({ originalDocument, owner })
    if (!extractedChunks.chunks || extractedChunks.chunks?.length == 0) throw new Error('No chunks extracted');
