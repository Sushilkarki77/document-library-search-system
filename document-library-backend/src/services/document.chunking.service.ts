...
async function generateOverlappingChunks(text: string, owner: Owner, originalDocument: OriginalDocument, groupSize = 3, overlap = 1) {
...
`;
  ...
  try {
    ...
    console.error("Failed to parse response:", content, err);
...
export async function handleDocumentChunking(msg: WorkerMessage): Promise<WorkerResponse> {
  try {
    const blob = await downloadFileBuffer(msg.originalDocument.filename);
    if (!blob) throw new Error("Error reading file");

    const extractedContents = await extractTextContents(blob);
    const chunks: Chunk[] = await generateOverlappingChunks(
      extractedContents.text,
      msg.owner,
      msg.originalDocument
    );
