...
export const getDocumentsByUserIdHandler: RequestHandler<
  unknown,
...
export const addDocumentHandler: RequestHandler<
  unknown,
  ResponseItem<
    Pick<Document, 'createdAt' | 'owner' | 'title' | 'filename'> & {
      uploadUrl: string;
    }
  >,
  { documentName: string; _id: string }
> = async (req, res, next) => {
  try {
    const user = req.user;
    ...
    res
      ...
      .json({
        ...
        data: { ...{ _id, createdAt, owner, title, filename }, uploadUrl },
      });
  } catch (error) {
    return next(error);
  }
...
export const deleteDocumentHandler: RequestHandler<
  { documentId: string },
  ResponseItem<Document>,
  unknown
> = async (req, res, next) => {
...
export const getURLDownloadURL: RequestHandler<
  { documentId: string },
  ResponseItem<{ downloadURL: string }>
> = async (req, res, next) => {
  try {
    const { documentId } = req.params;
    const user = req.user;
...
export const vectorizeDocumentHandler: RequestHandler<
  unknown,
  ResponseItem<boolean>,
  { documentId: string }
> = async (req, res, next) => {
  try {
    ...
    if (!documentId)
      throw new ErrorWithStatus('Doocument Id not provided', 400);
